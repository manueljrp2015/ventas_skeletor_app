...
class appOrdersController extends CI_Controller
{
	...
	*/
	...
	public function indexChangeAjust(){
	$data = [
			"folder"   => "orders",
			"file"     => "orders-adjust-courier",
			'listStoreTimes' => $this->appOrdersModel->getListStoreTimes(),
	    ];
	...
	public function getCostOrder(){
		$response = ["text" => json_encode( [
		        	"data" => $this->appOrdersModel->getCostOrder($this->input->get())])];
		        $this->load->view("app/response/text", $response);	
	}


	public function saveCostOrder(){

		if ($this->input->is_ajax_request()) {
			$response = ["text" => json_encode( [
		        	"data" => $this->appOrdersModel->saveCostOrder($this->input->post())])];
		        $this->load->view("app/response/text", $response);
		} else {
			$this->session->sess_destroy();
      		redirect('/','auto');
